  ...
  public static ResultSet displaychanged(String userid)throws ServletException, IOException, SQLException {
         ...
         Statement stmt;
           ResultSet rs;
            ...
            Connection con = getConnection();
            String queryString = "SELECT `userID`, `user_name`, `u_firstName`, `u_lastName`, `u_Phone`, `u_Email`,`u_PW` FROM `users` WHERE `userID`="+userid;
   ...
   public static int updatedetails(String username,String fname,String lname,String phone,String email,String id) throws SQLException{
      Statement stmt ;
        
       
        Connection con = getConnection();
       String queryString = "UPDATE `users` SET `user_name`='"+username+"',`u_firstName`='"+fname+"',`u_lastName`='"+lname+"',`u_Phone`='"+phone+"',`u_Email`='"+email+"' WHERE `userID`='"+id+"'";
   ...
   public static ResultSet display_user_req()throws ServletException, IOException, SQLException {
        ...
        int val=1;
            Connection con = getConnection();
            String queryString = "SELECT `userID`, `user_name`, `u_firstName`, `u_lastName`, `u_Phone`, `u_Email`,`u_PW` FROM `users` WHERE `u_status`="+val;
            stmt = con.createStatement();
             // query result is in rs object
            rs = stmt.executeQuery(queryString);
